...
router.get('/me', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
    ...
    try {
        ...
        try {
            ...
            if (role === 'CANDIDATE' && profile && !user.candidateProfile) {
                await db_1.prisma.candidateProfile.create({
                    data: {
                        userId: user.id,
                ...
                await db_1.prisma.recruiterProfile.create({
                    data: {
                        userId: user.id,
            ...
            if (upsertError.code === 'P2002' && upsertError.meta?.target?.includes('email')) {
                ...
                if (existingUser) {
                    ...
                    const finalUser = await db_1.prisma.user.findUnique({
                        where: { clerkId: userId },
                        include: {
                            recruiterProfile: true,
                            candidateProfile: true
    ...
    try {
        ...
        if (role === 'CANDIDATE' || user.role === 'CANDIDATE') {
            ...
            if (user.recruiterProfile) {
                // Update existing profile
                await db_1.prisma.recruiterProfile.update({
                    where: { userId: user.id },
                    data: {
...
router.post('/jobs', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        ...
        const { title, description, location, type, interviewContext, interviewEndDate, screenshotInterval, customQuestions } = req.body;
...
router.get('/my-jobs', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
// Get recruiter stats
router.get('/recruiter-stats', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
router.get('/jobs/:id/applications', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
...
router.post('/jobs/:id/apply', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        ...
        if (resumeBase64 && typeof resumeBase64 === 'string') {
            ...
            const { uploadResume } = await Promise.resolve().then(() => __importStar(require('../services/imageKitService')));
...
router.patch('/applications/:id/status', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
...
router.get('/my-interviews', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
router.delete('/jobs/:id', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
        ...
        const activeInterviews = job.applications.some(app => app.interview && !app.interview.endedAt);
        if (activeInterviews) {
...
router.put('/jobs/:id', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
        const jobId = req.params.id;
        if (!userId) {
...
router.get('/ai-templates', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const templates = [
            {
                id: 'technical-focus',
...
router.get('/interviews/:interviewId/recordings', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
