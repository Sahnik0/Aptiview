...
const saveRecording = async (recordingData, uniqueLink) => {
    ...
    const res = await (0, imageKitService_1.uploadBuffer)({
        buffer,
...
router.post('/schedule', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
...
router.post('/:uniqueLink/start', async (req, res) => {
    try {
        const { uniqueLink } = req.params;
        const interview = await prisma.interview.findUnique({
            where: { uniqueLink },
...
router.post('/:uniqueLink/chat', async (req, res) => {
    try {
        const { uniqueLink } = req.params;
...
// Upload screenshot
router.post('/:uniqueLink/screenshot', async (req, res) => {
    try {
        const { uniqueLink } = req.params;
        ...
        // Save screenshot
        const imageUrl = await (0, fileService_1.saveBase64Screenshot)(imageData, uniqueLink);
...
// End interview
router.post('/:uniqueLink/end', async (req, res) => {
    try {
        const { uniqueLink } = req.params;
...
router.get('/:uniqueLink/results', requireClerkAuth_1.requireClerkAuth, async (req, res) => {
    try {
        const userId = req.clerkUserId;
        const { uniqueLink } = req.params;
        if (!userId) {
